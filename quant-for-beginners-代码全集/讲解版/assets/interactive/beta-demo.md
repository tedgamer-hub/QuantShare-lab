# Beta 动态演示 · Quant for Beginners

该文件把 HTML、CSS 和 JavaScript 组合为一个可独立打开的互动演示，因此按完整文件收录。

## 代码片段 01：Beta 动态演示 · Quant for Beginners

- 原始位置：`assets/interactive/beta-demo.html`
- 所属小节：Beta 动态演示 · Quant for Beginners
- 用途：这是一个可直接在浏览器打开的交互式教学页面，主题为“Beta 动态演示 · Quant for Beginners”。
...
<body>
  ...
  "SPY": [
    ...
    const STOCKS = ["AAPL", "MSFT", "GOOGL", "AMZN", "META", "NVDA", "TSLA", "AMD", "JPM"];
    const MARKET = "SPY";
    const N = 300;
    const RUN_WINDOW = 60;
    const MIN_DAYS = 20;
