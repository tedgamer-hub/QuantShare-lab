# 夏普比率动态演示 · Quant for Beginners

该文件把 HTML、CSS 和 JavaScript 组合为一个可独立打开的互动演示，因此按完整文件收录。

## 代码片段 01：夏普比率动态演示 · Quant for Beginners

- 原始位置：`assets/interactive/sharpe-ratio-demo.html`
- 所属小节：夏普比率动态演示 · Quant for Beginners
- 用途：这是一个可直接在浏览器打开的交互式教学页面，主题为“夏普比率动态演示 · Quant for Beginners”。
...
<body>
  <div class="app">
    ...
    <div class="layout">
      <aside class="left">
        <div class="card">
          <div class="section-title">逐步代入公式 · r_f = 4%</div>
  ...
  "SPY": [
    ...
    const TICKERS = ["AAPL", "MSFT", "GOOGL", "AMZN", "META", "NVDA", "TSLA", "AMD", "JPM", "SPY"];
    const RF = 0.04;
    const TD = 252;
    const N = 300;
    const RUN_WINDOW = 60;
