...
## 代码片段 01：Generate README showcase images (run from repo root).

- 原始位置：`scripts/generate_showcase_images.py`
- 所属小节：仓库维护与内容生成
- 用途：自动化生成、整理或发布仓库中的教学材料，减少手工维护 Notebook、图片和交互页面的工作量。
- 处理过程：导入本段所需的计算、数据处理或绘图库；通过 yfinance 从 Yahoo Finance 下载真实证券日线行情；使用滚动窗口计算移动平均线等时序指标；从价格序列计算相邻交易日收益率；把逐日收益复合为累计收益或净值曲线；生成交易信号或持仓序列，为策略回测提供规则。
- 运行结果：终端/单元格文字摘要、写入磁盘的文件。
- 代码特点：环境准备、联网数据、时间序列、收益率计算、净值计算
- 主要依赖：__future__、warnings、pathlib、matplotlib.pyplot、numpy、pandas、yfinance、matplotlib
