...
## 代码片段 01：为 phase1_intro 下所有 notebook 的代码行补充中文行尾注释。

- 原始位置：`scripts/comment_phase1_notebooks.py`
- 所属小节：仓库维护与内容生成
- 用途：自动化生成、整理或发布仓库中的教学材料，减少手工维护 Notebook、图片和交互页面的工作量。
- 处理过程：导入本段所需的计算、数据处理或绘图库；通过 yfinance 从 Yahoo Finance 下载真实证券日线行情；通过 AkShare 获取 A 股历史行情并统一字段格式；用随机数模拟价格、收益率或概率路径；使用滚动窗口计算移动平均线等时序指标；把逐日收益复合为累计收益或净值曲线。
- 运行结果：图表、Notebook 表格、终端/单元格文字摘要。
- 代码特点：环境准备、联网数据、随机模拟、时间序列、净值计算
- 主要依赖：__future__、json、re、pathlib、numpy、pandas、matplotlib、yfinance
