...
## 代码片段 01：生成方差修正四种公式合一的动态演示 HTML。

- 原始位置：`scripts/build_variance_formulas_demo.py`
- 所属小节：仓库维护与内容生成
- 用途：自动化生成、整理或发布仓库中的教学材料，减少手工维护 Notebook、图片和交互页面的工作量。
- 处理过程：导入本段所需的计算、数据处理或绘图库；计算标准差或年化波动率以刻画风险；通过循环批量处理多个标的、参数或交易区间；打印关键数值，便于核验中间结果和最终指标。
- 运行结果：终端/单元格文字摘要、写入磁盘的文件。
- 代码特点：环境准备、波动分析、批量处理、结果展示
- 主要依赖：math、pathlib
