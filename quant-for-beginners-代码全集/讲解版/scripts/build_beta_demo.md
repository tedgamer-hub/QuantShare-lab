...
## 代码片段 01：生成 Beta 动态演示 HTML（数据来自 AkShare 美股日线）。

- 原始位置：`scripts/build_beta_demo.py`
- 所属小节：仓库维护与内容生成
- 用途：自动化生成、整理或发布仓库中的教学材料，减少手工维护 Notebook、图片和交互页面的工作量。
- 处理过程：导入本段所需的计算、数据处理或绘图库；通过 AkShare 获取美股历史行情并整理日期与价格字段；从价格序列计算相邻交易日收益率；利用协方差与方差计算 Beta，衡量相对大盘的敏感度；封装可复用函数，减少后续单元中的重复计算；通过循环批量处理多个标的、参数或交易区间。
- 运行结果：终端/单元格文字摘要、写入磁盘的文件。
- 代码特点：环境准备、联网数据、收益率计算、风险指标、函数封装
- 主要依赖：json、pathlib、akshare、pandas
