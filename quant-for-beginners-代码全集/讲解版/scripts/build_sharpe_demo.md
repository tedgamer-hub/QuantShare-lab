...
## 代码片段 01：生成夏普比率动态演示 HTML（数据来自 AkShare 美股日线）。

- 原始位置：`scripts/build_sharpe_demo.py`
- 所属小节：仓库维护与内容生成
- 用途：自动化生成、整理或发布仓库中的教学材料，减少手工维护 Notebook、图片和交互页面的工作量。
- 处理过程：导入本段所需的计算、数据处理或绘图库；通过 AkShare 获取美股历史行情并整理日期与价格字段；从价格序列计算相邻交易日收益率；计算夏普比率，用单位风险对应的超额收益评价表现；封装可复用函数，减少后续单元中的重复计算；通过循环批量处理多个标的、参数或交易区间。
